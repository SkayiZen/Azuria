package fr.skayizen.azuria.Events.Cookies.RandomLegendary;

import fr.minuskube.inv.ClickableItem;
import fr.minuskube.inv.SmartInventory;
import fr.minuskube.inv.content.InventoryContents;
import fr.minuskube.inv.content.InventoryProvider;
import fr.skayizen.azuria.Events.Cookies.RandomLegendaySH.RandomLegendaySH;
import fr.skayizen.azuria.Events.Cookies.RandomSH.RandomSH;
...
public class RandomLegendary implements InventoryProvider {
    public static final SmartInventory INVENTORY = SmartInventory.builder()
            .id("CookieLegendary")
            .title("§6§lCookie Random Légendaire !")
            .provider(new RandomLegendary())
            .size(5, 9)
            .closeable(true)
            .build();
    ...
    public void init(Player player, InventoryContents inventoryContents) {
        ...
        ItemStack yes = new ItemBuilder(greenGGemme)
                ...
                .withLoreColoured(Arrays.asList("§f", "§fUne fois clicker dessus, tu", "§frecevras un Pokemon Légendaire aléatoire. "))
                .build();
        inventoryContents.set(2, 3, ClickableItem.of(yes, e -> {
            ...
            Bukkit.dispatchCommand(Bukkit.getServer().getConsoleSender(), "pokegive " + player.getName() + " " + randomElement);
            player.sendMessage("§6Cookie §f» Vous avez utiliser votre Cookie Random Légendaire !");
        ...
        inventoryContents.set(2, 5, ClickableItem.of(no, e -> {
            player.sendMessage("§6Cookie §f» Vous avez annuler le procéssus de votre Cookie Random Légendaire !");
            RandomSH.INVENTORY.close(player);
